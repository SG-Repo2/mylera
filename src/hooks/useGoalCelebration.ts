...
export const useGoalCelebration = (userId: string | undefined) => {
  ...
  const getCelebrationKey = useCallback((date: string) => {
  ...
  const loadCelebratedMetrics = useCallback(async () => {
    ...
    try {
      ...
      if (stored) {
        ...
        const validCelebrations = celebrations.filter(c => 
  ...
  const saveCelebratedMetric = useCallback(async (metricType: MetricType) => {
    if (!userId) return;
    try {
      const today = new Date().toISOString().split('T')[0];
      const key = getCelebrationKey(today);
  ...
  const checkAndCelebrateGoal = useCallback(async (
    metricType: MetricType,
    value: number,
