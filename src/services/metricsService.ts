...
export const metricsService = {
  ...
  async getDailyMetrics(userId: string, date: string) {
    try {
      logger.debug(LogCategory.Metrics, 'Getting daily metrics', undefined, undefined, {
        userId,
        date,
      });
      ...
      logger.debug(LogCategory.Metrics, 'Retrieved daily metrics', undefined, undefined, {
        count: data?.length || 0,
      });
      return data || [];
    } catch (error) {
  ...
  async getDailyTotals(date: string) {
    try {
      logger.debug(
        LogCategory.Metrics,
        'Getting daily totals for leaderboard',
        undefined,
        undefined,
        { date }
      );
      ...
      logger.debug(LogCategory.Metrics, 'Retrieved daily totals', undefined, undefined, {
        count: data?.length || 0,
      });
      return data || [];
    } catch (error) {
  ...
  async getWeeklyLeaderboard(weekStart: string) {
    try {
      logger.debug(LogCategory.Metrics, 'Getting weekly leaderboard', undefined, undefined, {
        weekStart,
      });
      ...
      logger.debug(LogCategory.Metrics, 'Retrieved weekly leaderboard', undefined, undefined, {
        count: data?.length || 0,
      });
      return data || [];
    } catch (error) {
  ...
  async updateMetric(
    ...
    try {
      ...
      if (
        error instanceof MetricsAuthError ||
   ...
   */
  async updateMetrics(userId: string, metrics: Partial<Record<MetricType, number>>) {
    try {
   ...
   */
  async getMetricStreaks(userId: string, metricType: MetricType, minStreakLength: number = 2) {
    try {
      ...
      const { data, error } = await supabase.rpc('get_metric_streaks', {
        p_user_id: userId,
