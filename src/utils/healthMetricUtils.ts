  ...
  try {
    ...
    const validMetrics = metrics.filter(metric => {
      ...
      if (!isValid) {
        logger.warn(
          LogCategory.Health,
          `[healthMetricUtils] Filtering out invalid ${metric.type} value: ${metric.value}`
        );
      }
      return isValid;
    });
    ...
    logger.error(
      LogCategory.Health,
      '[healthMetricUtils] Error normalizing metrics:',
...
export function aggregateMetrics(metrics: NormalizedMetric[]): number {
  try {
    ...
    logger.debug(
      LogCategory.Health,
      `[healthMetricUtils] Aggregating metrics: ${JSON.stringify({
    ...
    if (metric.type === 'heart_rate') {
      ...
      const validHeartRates = metrics
        .filter(
          m =>
            ...
            m.value >= METRIC_VALIDATION.HEART_RATE.MIN &&
            m.value <= METRIC_VALIDATION.HEART_RATE.MAX
        )
      ...
      if (validHeartRates.length <= 3) {
        ...
        logger.debug(
          LogCategory.Health,
          '[healthMetricUtils] Heart rate weighted average:',
          undefined,
          undefined,
          {
            recentAvg,
    ...
    logger.error(
      LogCategory.Health,
      '[healthMetricUtils] Error aggregating metrics:',
      (error as Error).message
    );
    return 0;
  }
...
export function calculatePoints(
  ...
  try {
    switch (type) {
      ...
      case 'heart_rate':
        ...
        if (value >= minIdeal && value <= maxIdeal) {
          ...
          const distFromIdeal = value < minIdeal ? minIdeal - value : value - maxIdeal;
    ...
    logger.debug(
      LogCategory.Health,
      '[healthMetricUtils] Calculated points:',
      undefined,
      undefined,
      {
        type,
    ...
    logger.error(
      LogCategory.Health,
      '[healthMetricUtils] Error calculating points:',
      (error as Error).message
    );
    return { points: 0, goalReached: false };
  }
...
export function calculateHealthScore(
  ...
  logger.debug(
    LogCategory.Health,
    '[healthMetricUtils] Calculated health score:',
    undefined,
    undefined,
    { score }
  );
