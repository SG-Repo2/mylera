...
jest.mock('../services/scoreCalculatorService', () => ({
...
describe('unifiedMetricsService', () => {
  ...
  describe('synchronization', () => {
    ...
    it('verifies metric updates correctly', async () => {
      ...
      try {
        // Mock the database response with matching values
        const mockVerifiedMetrics = [
          createMetricScore(mockUserId, mockDate, 'steps', 1500, true, 15, 10000),
          createMetricScore(mockUserId, mockDate, 'heart_rate', 75, true, 10, 60),
          createMetricScore(mockUserId, mockDate, 'flights_climbed', 12, true, 12, 10)
        ];
        ...
        // Mock the Supabase query chain with retry behavior
        ...
        // Execute the verification with proper timing
        await act(async () => {
          // Start the verification
          const verificationPromise = unifiedMetricsService.verifyMetricUpdates(
            mockNativeMetrics,
            mockUserId
          );
    ...
    }, 50000); // Increase timeout for complex async operations
  });
