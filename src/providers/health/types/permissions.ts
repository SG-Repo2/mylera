...
export async function cachePermissionState(userId: string, state: PermissionState): Promise<void> {
  try {
...
export async function getCachedPermissionState(userId: string): Promise<PermissionState | null> {
  try {
    const key = getPermissionCacheKey(userId);
