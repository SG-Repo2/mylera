...
class TestBatchProvider extends BaseHealthProvider {
  ...
  async fetchRawMetrics(
    ...
    types.forEach(type => {
      if (type === 'steps') {
        mockData.steps = [
          { startDate: startDate.toISOString(), endDate: endDate.toISOString(), value: 5000, unit: 'count' }
        ];
      }
      if (type === 'distance') {
        mockData.distance = [
          { startDate: startDate.toISOString(), endDate: endDate.toISOString(), value: 3000, unit: 'meters' }
        ];
      }
      if (type === 'heart_rate') {
        mockData.heart_rate = [
          { startDate: startDate.toISOString(), endDate: endDate.toISOString(), value: 75, unit: 'bpm' }
        ];
      }
    });
  ...
  async getMetrics(): Promise<HealthMetrics> {
    ...
    return this.testBatchFetch(
      startOfDay,
      now,
      ['steps', 'distance', 'heart_rate'] as MetricType[]
    );
  }
...
describe('batchFetchHealthMetrics', () => {
  ...
  it('should handle errors in metric processing', async () => {
    ...
    // Execute
    const result = await provider.testBatchFetch(startDate, endDate, ['steps', 'distance', 'heart_rate'] as MetricType[]);
