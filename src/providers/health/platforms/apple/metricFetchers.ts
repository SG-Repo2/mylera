...
export async function fetchStepsRaw(options: HealthInputOptions): Promise<RawHealthMetric[]> {
  try {
    ...
    const results = await retryHealthKitOperation(
      () => promisify<StepCountResult[]>(AppleHealthKit.getDailyStepCountSamples, options),
      2,
      500
    );
    ...
    logger.error(
      LogCategory.Health,
      '[AppleHealthProvider] Error reading steps:',
...
export async function fetchDistanceRaw(options: HealthInputOptions): Promise<RawHealthMetric[]> {
  try {
    logger.debug(
      LogCategory.Health,
      '[AppleHealthProvider] Fetching distance with options:',
      undefined,
      undefined,
      options
    );
    ...
    try {
      const dailyResults = await retryHealthKitOperation(
        () =>
          promisify<DailySampleResult[]>(AppleHealthKit.getDailyDistanceWalkingRunningSamples, {
            ...options,
      ...
      console.warn(
        '[AppleHealthProvider] Error using daily distance samples, falling back to standard API:',
        dailyError
      );
    }
    ...
    logger.error(
      LogCategory.Health,
      '[AppleHealthProvider] Error reading distance:',
      (error as Error).message
    );
    return [];
  }
...
export async function fetchCaloriesRaw(options: HealthInputOptions): Promise<RawHealthMetric[]> {
  try {
    ...
    const results = await promisify<DailySampleResult[]>(AppleHealthKit.getActiveEnergyBurned, {
      ...options,
 ...
 */
export async function fetchBasalCaloriesRaw(
  ...
  try {
    ...
    logger.error(
      LogCategory.Health,
      '[AppleHealthProvider] Error reading basal calories:',
...
export async function fetchHeartRateRaw(options: HealthInputOptions): Promise<RawHealthMetric[]> {
  try {
    ...
    const results = await promisify<HeartRateSample[]>(AppleHealthKit.getHeartRateSamples, {
      ...options,
      ascending: false,
    ...
    // Sort by date
    return dailyAverages.sort(
      (a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime()
    );
  } catch (error) {
    logger.error(
      LogCategory.Health,
      '[AppleHealthProvider] Error reading heart rate:',
      error instanceof Error ? error.message : String(error)
    );
    return [];
  }
 ...
 */
export async function fetchFlightsClimbedRaw(
  options: HealthInputOptions
): Promise<RawHealthMetric[]> {
  try {
    const results = await promisify<SimpleValueResult>(AppleHealthKit.getFlightsClimbed, options);
    return [
      {
        startDate: options.startDate || new Date().toISOString(),
        endDate: options.endDate || new Date().toISOString(),
    ...
    logger.error(
      LogCategory.Health,
      '[AppleHealthProvider] Error reading flights climbed:',
      error instanceof Error ? error.message : String(error)
    );
    return [];
  }
...
export async function fetchExerciseRaw(options: HealthInputOptions): Promise<RawHealthMetric[]> {
  try {
    ...
    logger.error(
      LogCategory.Health,
      '[AppleHealthProvider] Error reading exercise time:',
      error instanceof Error ? error.message : String(error)
    );
    return [];
  }
