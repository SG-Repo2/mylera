...
export async function fetchStepsWithDailyAggregation(
  ...
  try {
    const stepsResponse = await retryOperation(
    ...
    logger.error(LogCategory.Health, '[GoogleHealthProvider] Error fetching steps:', (error as Error).message);
    return [];
  }
}
...
export async function fetchDistanceWithDailyAggregation(
  ...
  try {
    ...
    logger.error(LogCategory.Health, '[GoogleHealthProvider] Error fetching distance:', (error as Error).message);
    return [];
  }
}
...
export async function fetchCaloriesWithDailyAggregation(
  ...
  try {
    ...
    logger.error(LogCategory.Health, '[GoogleHealthProvider] Error fetching calories:', (error as Error).message);
    return [];
  }
}
...
export async function fetchHeartRateWithDailyAggregation(
  ...
  try {
    ...
    (heartRateResponse.records as HeartRateRecord[]).forEach(record => {
      const validSamples = record.samples
        .filter(sample => {
          ...
          if (!isValid) {
            logger.warn(LogCategory.Health, '[GoogleHealthProvider] Invalid heart rate sample:', JSON.stringify(sample));
          }
    ...
    logger.error(LogCategory.Health, '[GoogleHealthProvider] Error fetching heart rate:', (error as Error).message);
    return [];
  }
}
...
export async function fetchBasalCaloriesWithDailyAggregation(
  ...
  try {
    ...
    logger.error(LogCategory.Health, '[GoogleHealthProvider] Error fetching basal calories:', (error as Error).message);
    return [];
  }
}
...
export async function fetchExerciseWithDailyAggregation(
  ...
  try {
    ...
    logger.error(LogCategory.Health, '[GoogleHealthProvider] Error fetching exercise:', (error as Error).message);
    return [];
  }
}
...
export async function fetchFloorsClimbedWithDailyAggregation(
  ...
  try {
    ...
    logger.error(LogCategory.Health, '[GoogleHealthProvider] Error fetching floors climbed:', (error as Error).message);
    return [];
  }
}
