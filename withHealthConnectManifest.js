...
const withHealthConnectManifest = (config) => {
  ...
  config = withAndroidManifest(config, (config) => {
    ...
    try {
      // Add Health Connect queries - properly at manifest level
      if (!manifest.queries) {
        manifest.queries = [{}];
      }
      ...
      // Add Health Connect package if not already present
        ...
        const mainActivity = app.activity.find(
          activity => activity['$'] && 
            (activity['$']['android:name'] === '.MainActivity' || 
             activity['$']['android:name'] === 'com.yourcompany.yourappname.MainActivity')
        );
...
<permissions>
    ...
    <permission name="android.permission.health.READ_HEART_RATE"/>
        ...
        console.error('Error creating health_permissions.xml:', e);
      }
