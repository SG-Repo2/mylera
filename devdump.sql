...
CREATE OR REPLACE FUNCTION "public"."calculate_weekly_totals"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  week_start_date DATE;
  weekly_points INTEGER;
  weekly_metrics INTEGER;
BEGIN
  -- Calculate the week start date (Sunday)
  week_start_date := get_week_start(NEW.date);
  
  -- Calculate aggregate values for the week
  SELECT 
    COALESCE(SUM(points), 0),
    COUNT(DISTINCT metric_type)
  INTO 
    weekly_points,
    weekly_metrics
  FROM 
  ...
  WHERE 
    ...
    AND is_test_data = NEW.is_test_data;
  
  -- Update or insert into weekly_totals
  ...
  DO UPDATE SET
    ...
    metrics_completed = EXCLUDED.metrics_completed,
    updated_at = now();
...
ALTER FUNCTION "public"."calculate_weekly_totals"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."handle_new_user"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- Create a simple initial profile - only the essentials
  INSERT INTO public.user_profiles (
    id,
    display_name,
    show_profile,
    ...
    NEW.id,
    COALESCE(NEW.raw_user_meta_data->>'displayName', 'User ' || substr(NEW.id::text, 1, 8)),
    true,
    now(),
    now()
  )
  ON CONFLICT (id) DO NOTHING; -- Don't update if it exists
...
CREATE OR REPLACE FUNCTION "public"."update_metrics_transaction"("updates" "jsonb") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  -- Insert or update records in daily_metric_scores using the JSONB input.
  INSERT INTO public.daily_metric_scores (
    user_id, date, metric_type, value, updated_at
  )
  SELECT
    x.user_id,
    x.date::date,              -- Cast date from text to date type
    x.metric_type,
    x.value::numeric,          -- Cast value to numeric type
    x.updated_at::timestamptz    -- Cast updated_at to timestamptz
  FROM jsonb_to_recordset(updates) AS x(
    user_id uuid,
    date text,                 -- Expecting date as 'YYYY-MM-DD'
    metric_type text,
    value numeric,
  ...
  DO UPDATE SET
    value = EXCLUDED.value,    -- Use the new value
    updated_at = EXCLUDED.updated_at;  -- Update the timestamp
EXCEPTION WHEN OTHERS THEN
  -- Log a warning and re-raise the exception to propagate the error.
  RAISE WARNING 'update_metrics_transaction failed: %', SQLERRM;
  RAISE;
END;
$$;


ALTER FUNCTION "public"."update_metrics_transaction"("updates" "jsonb") OWNER TO "postgres";
...
COMMENT ON TABLE "public"."daily_metric_scores" IS 'Individual health metric scores per user per day';



CREATE TABLE IF NOT EXISTS "public"."daily_metric_scores_backup" (
    "id" "uuid",
    "user_id" "uuid",
    "date" "date",
    "value" numeric,
...
ALTER TABLE "public"."daily_metric_scores_backup" OWNER TO "postgres";
...
COMMENT ON TABLE "public"."daily_totals" IS 'Aggregated daily scores for leaderboard display';



CREATE TABLE IF NOT EXISTS "public"."daily_totals_backup" (
    "id" "uuid",
    "user_id" "uuid",
    "date" "date",
    "total_points" integer,
    "metrics_completed" integer,
    "is_test_data" boolean,
    "created_at" timestamp with time zone,
    "updated_at" timestamp with time zone
);
...
ALTER TABLE "public"."daily_totals_backup" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."user_profiles" (
    ...
    CONSTRAINT "user_profiles_device_type_check" CHECK (("device_type" = ANY (ARRAY['OS'::"text", 'fitbit'::"text", 'os'::"text"]))),
...
COMMENT ON TABLE "public"."weekly_totals" IS 'Aggregated weekly scores for weekly leaderboard display';



CREATE TABLE IF NOT EXISTS "public"."weekly_totals_backup" (
    "id" "uuid",
    "user_id" "uuid",
    "week_start" "date",
    "total_points" integer,
...
ALTER TABLE "public"."weekly_totals_backup" OWNER TO "postgres";
...
CREATE INDEX "idx_daily_metric_scores_date" ON "public"."daily_metric_scores" USING "btree" ("date");



CREATE INDEX "idx_daily_metric_scores_metric_type" ON "public"."daily_metric_scores" USING "btree" ("metric_type");
...
CREATE INDEX "idx_daily_metric_scores_user_metric_type" ON "public"."daily_metric_scores" USING "btree" ("user_id", "metric_type");
...
CREATE INDEX "idx_daily_totals_total_points" ON "public"."daily_totals" USING "btree" ("total_points" DESC);
...
CREATE INDEX "idx_weekly_totals_total_points" ON "public"."weekly_totals" USING "btree" ("total_points" DESC);
...
CREATE INDEX "idx_weekly_totals_week" ON "public"."weekly_totals" USING "btree" ("week_start");
...
CREATE POLICY "daily_metric_scores_insert_own" ON "public"."daily_metric_scores" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "daily_metric_scores_select_own" ON "public"."daily_metric_scores" FOR SELECT USING (("auth"."uid"() = "user_id"));



CREATE POLICY "daily_metric_scores_update_own" ON "public"."daily_metric_scores" FOR UPDATE USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "daily_totals_select_all" ON "public"."daily_totals" FOR SELECT USING ((("auth"."uid"() = "user_id") OR (EXISTS ( SELECT 1
   FROM "public"."user_profiles"
  WHERE (("user_profiles"."id" = "daily_totals"."user_id") AND ("user_profiles"."show_profile" = true))))));



CREATE POLICY "daily_totals_select_own" ON "public"."daily_totals" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "user_profiles_select_public" ON "public"."user_profiles" FOR SELECT USING (("show_profile" = true));
...
CREATE POLICY "weekly_totals_select_all" ON "public"."weekly_totals" FOR SELECT USING ((("auth"."uid"() = "user_id") OR (EXISTS ( SELECT 1
   FROM "public"."user_profiles"
  WHERE (("user_profiles"."id" = "weekly_totals"."user_id") AND ("user_profiles"."show_profile" = true))))));



CREATE POLICY "weekly_totals_select_own" ON "public"."weekly_totals" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
GRANT ALL ON FUNCTION "public"."calculate_weekly_totals"() TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_weekly_totals"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_weekly_totals"() TO "service_role";



GRANT ALL ON FUNCTION "public"."check_auth_user_exists"("user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."check_auth_user_exists"("user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_auth_user_exists"("user_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."create_user_profile"("p_user_id" "uuid", "p_display_name" "text", "p_device_type" "text", "p_measurement_system" "text", "p_avatar_url" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."create_user_profile"("p_user_id" "uuid", "p_display_name" "text", "p_device_type" "text", "p_measurement_system" "text", "p_avatar_url" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."create_user_profile"("p_user_id" "uuid", "p_display_name" "text", "p_device_type" "text", "p_measurement_system" "text", "p_avatar_url" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."ensure_auth_user_exists"() TO "anon";
GRANT ALL ON FUNCTION "public"."ensure_auth_user_exists"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."ensure_auth_user_exists"() TO "service_role";



GRANT ALL ON FUNCTION "public"."generate_default_display_name"() TO "anon";
GRANT ALL ON FUNCTION "public"."generate_default_display_name"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."generate_default_display_name"() TO "service_role";



GRANT ALL ON FUNCTION "public"."get_metric_streaks"("p_user_id" "uuid", "p_metric_type" "text", "p_min_streak_length" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."get_metric_streaks"("p_user_id" "uuid", "p_metric_type" "text", "p_min_streak_length" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_metric_streaks"("p_user_id" "uuid", "p_metric_type" "text", "p_min_streak_length" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."get_week_start"("date_input" "date") TO "anon";
GRANT ALL ON FUNCTION "public"."get_week_start"("date_input" "date") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_week_start"("date_input" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."track_health_metric_changes"() TO "anon";
GRANT ALL ON FUNCTION "public"."track_health_metric_changes"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."track_health_metric_changes"() TO "service_role";



GRANT ALL ON FUNCTION "public"."update_daily_ranks"("target_date" "date") TO "anon";
GRANT ALL ON FUNCTION "public"."update_daily_ranks"("target_date" "date") TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_daily_ranks"("target_date" "date") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_metric_points"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_metric_points"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_metric_points"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_metrics_transaction"("updates" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."update_metrics_transaction"("updates" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_metrics_transaction"("updates" "jsonb") TO "service_role";



GRANT ALL ON FUNCTION "public"."update_profile_avatar"("p_user_id" "uuid", "p_avatar_url" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."update_profile_avatar"("p_user_id" "uuid", "p_avatar_url" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_profile_avatar"("p_user_id" "uuid", "p_avatar_url" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_weekly_totals_trigger"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_weekly_totals_trigger"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_weekly_totals_trigger"() TO "service_role";
...
GRANT ALL ON TABLE "public"."daily_metric_scores_backup" TO "anon";
GRANT ALL ON TABLE "public"."daily_metric_scores_backup" TO "authenticated";
GRANT ALL ON TABLE "public"."daily_metric_scores_backup" TO "service_role";
...
GRANT ALL ON TABLE "public"."daily_totals_backup" TO "anon";
GRANT ALL ON TABLE "public"."daily_totals_backup" TO "authenticated";
GRANT ALL ON TABLE "public"."daily_totals_backup" TO "service_role";
...
GRANT ALL ON TABLE "public"."weekly_totals_backup" TO "anon";
GRANT ALL ON TABLE "public"."weekly_totals_backup" TO "authenticated";
GRANT ALL ON TABLE "public"."weekly_totals_backup" TO "service_role";
